...
export default async function ScrapDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="mx-auto min-h-[calc(100dvh-4.5rem)] w-full max-w-[1280px] px-5 py-12 md:px-8 md:py-24">
      <Link
        href="/scraps"
      ...
      <article className="mt-12 grid gap-12 lg:grid-cols-12 lg:gap-x-8">
        <header className="lg:sticky lg:top-28 lg:col-span-4 lg:self-start">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 font-label text-[0.6875rem] tracking-[0.06em] text-tertiary uppercase">
            <span>
              {scrap.entryCount} {scrap.entryCount === 1 ? "post" : "posts"}
            </span>
          </div>
          <h1 className="mt-5 font-display text-[clamp(2.75rem,5vw,4.5rem)] leading-[1.08] font-bold tracking-[-0.035em] text-balance">
            {scrap.title}
          </h1>
          ...
          <dl className="mt-8 grid grid-cols-2 gap-5 border-t border-outline-variant pt-5 font-label text-[0.6875rem] text-tertiary lg:grid-cols-1">
            <div>
              <dt className="tracking-[0.06em] uppercase">Created</dt>
              <dd className="mt-1 text-on-surface-variant">
                <time dateTime={scrap.createdAt}>{formatScrapDate(scrap.createdAt)}</time>
              </dd>
            </div>
            <div>
              <dt className="tracking-[0.06em] uppercase">Updated</dt>
              <dd className="mt-1 text-on-surface-variant">
        ...
        <div className="lg:col-start-6 lg:col-span-7">
          <ol className="space-y-10">
            {entries.map((entry) => (
              ...
              >
                <div className="mb-7 flex items-center justify-between gap-4 border-b border-outline-variant pb-4 font-label text-[0.6875rem] text-tertiary">
